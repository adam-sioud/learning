...
class Person { 
	...
	{ 
		...
		for (Person person : personer.stream().filter(new PersonAge17Predicate()).toList()) {
			System.out.println(person);
		}

		for (Person person : personer.stream().filter(new PersonAgePredicate(50)).filter(new PersonAge17Predicate()).toList()) {
			System.out.println(person);
		}
	}
